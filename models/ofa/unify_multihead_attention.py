...
class MultiheadSpatialAttention(MultiheadAttention):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            qn_block_size, scale_factor, scale_heads
        )

        if encoder_decoder_attention:
            self.spatial_dim = self.head_dim
            # self.spatial_dim = 32
            h, w = 30, 30
    ...
    def forward(
        ...
        if self.encoder_decoder_attention:
            ...
            selected_atn_probs = attn_probs[:, :, :900].contiguous().view(bsz*self.num_heads, tgt_len, 30, 30)
            selected_atn_probs = selected_atn_probs.contiguous().view(bsz, self.num_heads, tgt_len, 30, 30).transpose(1, 2)
            ...
            dummy_data = torch.zeros(
                (bsz*self.num_heads, tgt_len, 8, self.spatial_dim),
                device=spatial.device, requires_grad=False, dtype=spatial.dtype
            )
        ...
        # print('weight', self.out_proj.weight.shape, self.additional_weight.shape)
        # print(torch.cat([self.out_proj.weight, self.additional_weight], axis=1).shape)
        # print(torch.cat([self.out_proj.weight, self.additional_weight], axis=1)[0, -40:])

        # w_1_detail = self.out_proj.weight.view(self.embed_dim, self.num_heads, self.head_dim)
        # w_2_detail = self.additional_weight.view(self.embed_dim, self.num_heads, self.spatial_dim)

        # combine_w = torch.cat([w_1_detail, w_2_detail], dim=-1).view(self.embed_dim, self.embed_dim + self.num_heads*self.spatial_dim)

        # attn = nn.functional.linear(
        #     attn,
        #     combine_w,
        #     # self.out_proj.weight,
        #     self.out_proj.bias
